...
class App:
    ...
    def __init__(self, master):
        ...
        self.Past_Grades = ttk.Button(frame, text='Load Past Students Grades', command=self.open_files).grid(row=2,
                                                                                                             ...
                                                                                                             pady=20,
                                                                                                             padx=5)
        self.Transcript = ttk.Button(frame, text='Load Your Current Transcript', command=self.open_file).place(relx=0.63,rely=0.3)
        ...
        #ComboBox
        self.similarity_type = Label(frame, text='Similarity Measure :').grid(row=3, column=5, pady=20)
        self.combobox_options = ['Pearson', 'Euclidean', 'Jaccard']
        self.box = ttk.Combobox(frame, textvariable=ComboVar, values=self.combobox_options, width=10).grid(row=3,
                                                                                                          column=10,
                                                                                                           pady=20,
                                                                                                           sticky=(W))
        ...
        self.get_recommendation = ttk.Button(frame, text='See The Recommended Courses',
                                             command=self.Build_recommend).grid(row=8, column=1, columnspan=2, pady=15)

    def open_files(self):
        """ ask for files from user
    ...
    def build_previous_data(self, files):
        ...
        for file in files:
            ...
            for row in range(1, info.nrows):
                ...
                CourseName = str(info.row(row)[1]).split("\'")[1]
